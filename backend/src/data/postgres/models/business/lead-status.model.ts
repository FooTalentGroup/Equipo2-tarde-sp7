...
export class LeadStatusModel {
    private static readonly TABLE_NAME = 'lead_status';

    static async create(data: CreateLeadStatusDto): Promise<LeadStatus> {
    ...
    static async findAll(): Promise<LeadStatus[]> {
        const client = PostgresDatabase.getClient();
    ...
    static async findById(id: string): Promise<LeadStatus | null> {
        const client = PostgresDatabase.getClient();
    ...
    static async findByTitle(title: string): Promise<LeadStatus | null> {
        const client = PostgresDatabase.getClient();
    ...
    static async update(id: string, updateData: Partial<CreateLeadStatusDto>): Promise<LeadStatus | null> {
        const client = PostgresDatabase.getClient();
        if (!updateData.title) {
