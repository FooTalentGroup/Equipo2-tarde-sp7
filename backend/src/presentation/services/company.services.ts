    ...
    async updateCompanyLogo(file: Express.Multer.File, userId: number) {
        try {
            ...
            if (!allowedMimeTypes.includes(file.mimetype)) {
                throw CustomError.badRequest('Invalid file format. Only JPG and PNG are allowed');
            }
            ...
            const logoUrl = await this.fileUploadAdapter.uploadFile(
                file.buffer,
                {
                    folder: 'company',
                    resourceType: 'image',
            ...
            const updatedSettings = await CompanySettingsModel.updateLogo(logoUrl, userId);
