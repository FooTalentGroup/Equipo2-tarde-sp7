...
export class PropertyServices {
  ...
  async createProperty(
    ...
    return await TransactionHelper.executeInTransaction(async () => {
      ...
      if (
        ...
        const ownerClient = await ClientModel.findById(
          createPropertyDto.owner_id
        );
        if (!ownerClient) {
          throw CustomError.badRequest(
            `Client with ID ${createPropertyDto.owner_id} not found. Please create a client first.`
          );
        }
      ...
      const [
        ...
        finalOwnerId
          ? ClientModel.findById(finalOwnerId)
  ...
  async createPropertyGrouped(
    ...
    return await TransactionHelper.executeInTransaction(async () => {
      ...
      if (basic.owner_id !== undefined && basic.owner_id !== null) {
        ...
        if (!ownerClient) {
          throw CustomError.badRequest(
            `Client with ID ${basic.owner_id} not found`
          );
        }
      ...
      console.log(
        `[PropertyServices] Processing images: ${
          images?.length || 0
        } images received`
      );
      ...
      if (images && images.length > 0) {
        try {
          for (let i = 0; i < images.length; i++) {
            const image = images[i];
            console.log(
              `[PropertyServices] Uploading image ${i + 1}/${images.length}: ${
                image.originalname
            ...
            console.log(
              `[PropertyServices] Image ${
                i + 1
              } uploaded successfully: ${imageUrl}`
            );
            uploadedImages.push(imageUrl);
            ...
            console.log(
              `[PropertyServices] Multimedia record created: ${JSON.stringify(
                multimedia
              )}`
            );
            multimediaRecords.push(multimedia);
   ...
   */
  async getPropertyHistory(id: number) {
    const history = await CrmInteractionModel.findHistoryByProperty(id);
    return history;
  }
}
