...
import type { CreateTenantDto } from "../../domain/dtos/clients/create-tenant.dto";
...
export class TenantServices {
	...
	async createTenantWithProperty(
		...
		return await TransactionHelper.executeInTransaction(async () => {
			...
			const categoryId =
				await ClientCreationHelper.resolveContactCategory("Inquilino");
			...
			if (createTenantDto.currency_type_id) {
				...
				let currencyType = await CurrencyTypeModel.findByName(
					createTenantDto.currency_type,
				);

				// Si no se encuentra por nombre, intentar por símbolo
				if (!currencyType || !currencyType.id) {
					currencyType = await CurrencyTypeModel.findBySymbol(
						createTenantDto.currency_type,
					);
				}
			...
			if (propertyId && createTenantDto.contract_start_date && newClient.id) {
				...
				const normalizedStartDate = normalizeDate(
					createTenantDto.contract_start_date,
				);
				const normalizedEndDate = normalizeDate(
					createTenantDto.contract_end_date,
				);
				const normalizedIncreaseDate = normalizeDate(
					createTenantDto.next_increase_date,
				);
				...
				if (
					createTenantDto.monthly_amount &&
					currencyTypeId &&
					clientRental.id
				) {
