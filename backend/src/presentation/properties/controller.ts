...
import { CustomError, CreatePropertyDto, CreatePropertyGroupedDto } from '../../domain';
...
export class PropertyController {
    ...
    getPropertyById = async (req: Request, res: Response) => {
        try {
            ...
            const result = await this.propertyServices.getPropertyById(Number(id), includeArchived);
    ...
    listProperties = async (req: Request, res: Response) => {
        try {
            ...
            if (property_status_id) filters.property_status_id = Number(property_status_id);
            if (visibility_status_id) filters.visibility_status_id = Number(visibility_status_id);
            if (owner_id) filters.owner_id = Number(owner_id);
            if (captured_by_user_id) filters.captured_by_user_id = Number(captured_by_user_id);
            ...
            if (operation_type_id) filters.operation_type_id = Number(operation_type_id);
            if (currency_type_id) filters.currency_type_id = Number(currency_type_id);
            if (featured_web !== undefined) filters.featured_web = featured_web === 'true';
    ...
    updateProperty = async (req: Request, res: Response) => {
        try {
            ...
            const result = await this.propertyServices.updateProperty(Number(id), updateData);
            return res.json({
    ...
    createPropertyGrouped = async (req: Request, res: Response) => {
        try {
            ...
            console.log('Files received:', {
                ...
                fileKeys: req.files && typeof req.files === 'object' ? Object.keys(req.files as object) : []
            });
            ...
            const [error, createPropertyGroupedDto] = CreatePropertyGroupedDto.create(req.body);
