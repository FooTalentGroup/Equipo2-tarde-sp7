...
export class UpdateProfileDto {
    ...
    static create( object: { [key: string]: any }): [string?, UpdateProfileDto?] {
        const { first_name, last_name, email, phone, role_id, whatsapp_number } = object;
        
        if (!first_name && !last_name && !email && !phone && !role_id && !whatsapp_number) {
        ...
        if (whatsapp_number !== undefined && whatsapp_number !== null) {
            if (whatsapp_number.trim().length > 0 && !regularExps.whatsapp.test(whatsapp_number.trim())) {
